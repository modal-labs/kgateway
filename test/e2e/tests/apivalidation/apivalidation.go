...
// Package apivalidation holds the body of the TestAPIValidation e2e
// test.
...
// Run executes the TestAPIValidation scenario against the Installation produced
// by the supplied factory. Only the CRD chart is installed; the test exercises
// CRD-level validation rules by applying YAML and asserting on apply errors.
func Run(t *testing.T, factory e2e.InstallationFactory) {
	ctx := t.Context()
	ti := factory(t, &install.Context{
	...
	tests := []struct {
		...
		// wantErrors are treated as regular expressions, so any special
...
`,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			testutils.Cleanup(t, func() {
				ti.ActionsProvider().Kubectl().DeleteFile(ctx, tc.input) //nolint:errcheck
			})

			out := new(bytes.Buffer)

			err := ti.ActionsProvider().Kubectl().WithReceiver(out).Apply(ctx, []byte(tc.input))
